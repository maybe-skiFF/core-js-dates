...
function getNextFriday(date) {
  ...
  let fridayBuffer = 5 - curDay;
  if (fridayBuffer <= 0) {
    fridayBuffer += 7;
  }
  return new Date(date.setDate(fridayBuffer + date.getDate()));
}
...
function isDateInPeriod(date, period) {
  ...
  if (new Date(date) >= startPeriod && new Date(date) <= endPeriod) return true;
  return false;
}
...
function formatDate(date) {
  ...
  if (hours >= 0 && hours < 12) formatTime = 'AM';
  else if (hours > 12) {
    formatTime = 'PM';
    hours -= 12;
  } else if (hours === 12) {
  ...
  return `${month}/${day}/${year}, ${hours}:${formatMinutes}:${formatSeconds} ${formatTime}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysPerMonth; i += 1) {
    const iterableDate = new Date(year, month - 1, i);
    if (iterableDate.getDay() === 0 || iterableDate.getDay() === 6) {
      countOfWekend += 1;
    }
  }
  return countOfWekend;
}
...
function getNextFridayThe13th(date) {
  const dateOfNextFriday13 = date;

  if (dateOfNextFriday13.getDate() < 13) {
    dateOfNextFriday13.setDate(13);

    while (dateOfNextFriday13.getDay() !== 5) {
      dateOfNextFriday13.setMonth(dateOfNextFriday13.getMonth() + 1);
    }
  }
  return dateOfNextFriday13;
}
